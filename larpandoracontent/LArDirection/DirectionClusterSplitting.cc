/**
 *  @file   larpandoracontent/LArVertex/DirectionClusterSplitting.cc
 * 
...
void DirectionClusterSplitting::CountClusters()
{
...
void DirectionClusterSplitting::SelectClusters(ClusterVector &clusterVectorU, ClusterVector &clusterVectorV, ClusterVector &clusterVectorW)
{
    for (const std::string &clusterListName : m_inputClusterListNames)
    {
        const ClusterList *pClusterList(NULL);
        ...
        if (!pClusterList || pClusterList->empty())
        {
            if (PandoraContentApi::GetSettings(*this)->ShouldDisplayAlgorithmInfo())
                std::cout << "DirectionClusterSplitting: unable to find cluster list " << clusterListName << std::endl;
...
const pandora::Cluster* DirectionClusterSplitting::GetTargetCluster(ClusterVector &clusterVectorW)
{
...
std::pair<const pandora::CaloHit*, const pandora::Cluster*> DirectionClusterSplitting::RetrieveSplitCaloHitClusterPair(const pandora::Cluster* pCluster, TrackDirectionTool::DirectionFitObject &fitResult)
{
...
std::pair<const pandora::CaloHit*, const pandora::Cluster*> DirectionClusterSplitting::FindMatchingCaloHitClusterPair(TrackDirectionTool::DirectionFitObject &fitResult, ClusterVector &clusterVector)
{
...
pandora::StatusCode DirectionClusterSplitting::SplitCluster(std::pair<const pandora::CaloHit*, const pandora::Cluster*> hitClusterPair, pandora::HitType listHitType)
{
    for (const std::string &clusterListName : m_inputClusterListNames)
    {
        const ClusterList *pClusterList(NULL);
...
void DirectionClusterSplitting::DivideCaloHits(const pandora::Cluster* pCluster, const pandora::CaloHit* pTargetCaloHit, CaloHitList &caloHitList1, CaloHitList &caloHitList2)
{
    OrderedCaloHitList orderedCaloHitList(pCluster->GetOrderedCaloHitList());
    CaloHitList caloHitList;
    orderedCaloHitList.FillCaloHitList(caloHitList);    
...
StatusCode DirectionClusterSplitting::ReadSettings(const TiXmlHandle xmlHandle)
{
